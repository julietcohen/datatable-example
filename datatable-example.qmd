...
```{r}
tic()
ftm_trees_df <- readr::read_csv("FTM_trees.csv") # readr is the tidyverse way to read in csv files
toc()       
```

```{r}
tic()
ftm_trees_dt <- data.table::fread("FTM_trees.csv") # fread works the same way
toc()
```
...
Wrangle the dataset to only include ponderosa species of trees 
...
```{r}
class(lazy_trees) # under the hood, the lazy_dt() function is the first step to use dplyr syntax but R converts it into base R so it runs faster
```
...
ftm_trees_df %>% 
  ...
  summarize(mean_height = mean(HT_m, na.rm = TRUE),
            mean_diameter = mean(DBH_cm, na.rm = TRUE)) %>% 
  as_tibble() # converts the obj into a class we are familiar with
```
